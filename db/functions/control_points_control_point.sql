-- TODO: Schema muss irgendiw noch parametrisierbar gemacht werden?
-- SEARCH PATH o.ä.?
CREATE OR REPLACE FUNCTION av_mopublic_export.control_points_control_point(dbschema text, bfsnr integer)
  RETURNS boolean AS
$$
BEGIN
  RAISE NOTICE 'Insert data from query into table control_points_control_point.';
...
SELECT nextval('''||dbschema||'.t_id'')  AS t_id, kategorie as category, nbident as identnd, nummer as anumber,
       ...
       to_timestamp(stand_am, ''YYYYMMDD'')::timestamp without time zone as state_of,
